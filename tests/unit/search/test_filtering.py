"""Unit tests for search filtering behavior through the engine.
...
from search.app.vectorstore import milvus_store as milvus_mod
...
def _doc(
    ...
    # Commercial metadata mirrors the catalog CSV: `url` is present per row,
    # `rating` only when the catalog ever carries it.
    if url is not None:
        metadata["url"] = url
...
class TestMilvusAdapter:
    def test_add_and_search_uses_pymilvus_boundary(
        ...
        monkeypatch.setattr(milvus_mod, "Collection", _FakeCollection)
...
class TestRetrieve:
    async def test_text_only_category_match_filters(self, retriever: Retriever) -> None:
        ...
        # D4 commercial fields flow out of retrieve() alongside the legacy ones.
        assert urls == [""]
        assert prices == [49.99]
        assert ratings == [None]

    async def test_text_only_no_categories_returns_unfiltered(
        self, retriever: Retriever
    ) -> None:
        retriever.text_db.similarity_search_with_relevance_scores = MagicMock(
    ...
    async def test_empty_and_blank_categories_do_not_filter(
        self, retriever: Retriever
    ) -> None:
        search = MagicMock(return_value=[(_doc("Any", category="bag"), 0.9)])
        ...
        search.assert_called_once_with("q", k=4, expr=None)
    ...
    async def test_category_prefilter_survives_polluted_top_k(
        self, retriever: Retriever
    ) -> None:
        retriever.category_prefilter_k = 50
        polluted_results = [(_doc(f"External {idx}", category="toy"), 0.95) for idx in range(4)]
        polluted_results.append((_doc("Summer Dress", category="dress"), 0.7))

        def _search(_query: str, k: int = 4, expr: str | None = None) -> List[Tuple[Any, float]]:
            return polluted_results[:k]
        ...
        assert names == ["Summer Dress"]
        assert "CATEGORY_MISMATCH" not in texts[0]
        search.assert_called_once_with("夏季裙子", k=50, expr='subcategory in ["dress"]')

    async def test_expanded_candidate_pool_is_clamped(
        self, retriever: Retriever
    ) -> None:
        retriever.category_prefilter_k = 500
        search = MagicMock(return_value=[(_doc("Any", category="dress"), 0.9)])
        retriever.text_db.similarity_search_with_relevance_scores = search
        ...
        assert search.call_args.kwargs["k"] == 100

    async def test_empty_category_filter_falls_back_with_mismatch_marker(
        self, retriever: Retriever
    ) -> None:
        retriever.text_db.similarity_search_with_relevance_scores = MagicMock(
            return_value=[
                (_doc("External Toy", category="toy"), 0.95),
                (_doc("Near Match", category="skirt"), 0.75),
            ]
        )
        ...
        assert names == ["External Toy"]
        assert "CATEGORY_MISMATCH: true" in texts[0]
        assert prices == [49.99]
        assert sims == [0.95]

    async def test_category_fallback_is_capped(
        self, retriever: Retriever
    ) -> None:
        retriever.text_db.similarity_search_with_relevance_scores = MagicMock(
            return_value=[
                (_doc(f"External {idx}", category="toy"), 0.99 - idx * 0.01)
                for idx in range(5)
            ]
        )
        ...
        assert names == ["External 0", "External 1"]
    ...
    async def test_image_search_applies_category_filter(
        self, retriever: Retriever
    ) -> None:
        retriever.text_db.similarity_search_with_relevance_scores = MagicMock(
        ...
        retriever.image_db.similarity_search_with_relevance_scores = MagicMock(
            return_value=[
                (_doc("Image Match", category="bag", url="/images/image_match.jpg", rating=4.5), 0.95)
            ]
        )
        ...
        assert "Text Match" in names
        retriever.image_db.similarity_search_with_relevance_scores.assert_called_once_with(
            "data:image/jpeg;base64,AAA",
            k=50,
            expr='subcategory in ["dress"]',
        )
    ...
    async def test_blank_query_replaced_with_dummy(
        ...
        def _search(query: str, k: int = 4, expr: str | None = None) -> List[Tuple[Any, float]]:
...
class TestMilvusCategoryExpr:
    def test_builds_single_and_multiple_category_expr(self) -> None:
        assert Retriever._milvus_category_expr(["dress"]) == 'subcategory in ["dress"]'
        assert (
            Retriever._milvus_category_expr(["Dress", "Summer Skirt "])
            == 'subcategory in ["dress", "summer skirt"]'
        )

    def test_escapes_quotes_and_backslashes(self) -> None:
        assert (
            Retriever._milvus_category_expr(['ca"te\\gory'])
            == 'subcategory in ["ca\\"te\\\\gory"]'
        )

    def test_empty_categories_produce_no_filter(self) -> None:
        assert Retriever._milvus_category_expr([]) == ""
        assert Retriever._milvus_category_expr(["", "  "]) == ""
