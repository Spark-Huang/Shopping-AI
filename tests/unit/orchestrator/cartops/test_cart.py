...
"""Unit tests for ``orchestrator.app.cart``.
...
from orchestrator.app.agents.cartops import catalog_match
from orchestrator.app.agents.cartops import memory_client
from orchestrator.app.agents.state import Cart, State
from orchestrator.app.agents.cartops.agent import CartAgent
from orchestrator.app.agents.cartops.catalog_match import (
    _extract_price,
    _normalize_name,
...
class TestFindNamedProduct:
    ...
    def test_generic_word_collision_does_not_create_named_anchor(self) -> None:
        # PM review 2026-08-24, item 13 regression: "add a silk scarf to
        # my cart" must NOT latch onto the chat-heading span
        # "**Manage your cart**" just because both share the generic word
        # "cart". One shared filler token is not a product reference, so
        # no deterministic "named" anchor is produced (the LLM's correct
        # pick survives).
        assert (
            CartAgent._find_named_product(
                "add a silk scarf to my cart", ["Manage your cart"]
            )
            is None
        )

    def test_one_sided_coverage_below_half_rejected(self) -> None:
        # Query covers < 50% of the candidate and the candidate covers
        # < 50% of the query's catalog tokens: both directions must clear
        # 0.5 (unless the query is a deliberate abbreviation, see below).
        assert (
            CartAgent._find_named_product(
                "add the scarf to my cart", ["Silk Dress", "Leather Bag", "Cart Wheel Bag"]
            )
            is None
        )

    def test_deliberate_abbreviation_still_resolves(self) -> None:
        # q_cov == 1.0 (every catalog token of the query belongs to this
        # candidate): bulk-style abbreviated references must still
        # resolve to the full catalog name.
        assert (
            CartAgent._find_named_product(
                "honey skirt", ["Honey Floral Print Midi Skirt", "Lace Blouse"]
            )
            == "Honey Floral Print Midi Skirt"
        )


class TestLooksLikeProductNameProseGuard:
    """Commerce/UI prose spans must not become product anchors."""
    ...
    def test_rejects_ui_prose_spans(self, value: str) -> None:
        assert CartAgent._looks_like_product_name(value) is False
    ...
    def test_accepts_real_product_names(self, value: str) -> None:
        # None of the guard terms appear in real catalog names.
        assert CartAgent._looks_like_product_name(value) is True
...
# ---------------------------------------------------------------------------
# Ordinal helpers (pure functions)
# ---------------------------------------------------------------------------


class TestStripOrdinals:
    @pytest.mark.parametrize(
        "query,expected",
        [
            ("add the first one to my cart", "add the   one to my cart"),
            ("grab the 2nd dress", "grab the   dress"),
            ("what about the last", "what about the  "),
            ("add silk dress", "add silk dress"),  # untouched
        ],
    )
    def test_ordinals_removed(self, query: str, expected: str) -> None:
        assert cart_mod._strip_ordinals(query) == expected


class TestExtractOrdinalPosition:
    @pytest.mark.parametrize(
        "query,expected",
        [
            ("add the first one to my cart", 1),
            ("take the 1st item", 1),
            ("add the second one", 2),
            ("that 2nd dress please", 2),
            ("the third option", 3),
            ("give me the 3rd", 3),
            ("add the fourth one", 4),
            ("the 5th element", 5),
            ("I'll take the last", -1),
            ("no ordinals here", None),
            ("", None),
        ],
    )
    def test_positions(self, query: str, expected: Any) -> None:
        assert cart_mod._extract_ordinal_position(query) == expected


# ---------------------------------------------------------------------------
# Display-order extraction (pure logic)
# ---------------------------------------------------------------------------


class TestExtractDisplayOrder:
    def test_empty_context_returns_empty(self, cart_agent: CartAgent) -> None:
        assert CartAgent._extract_display_order("") == []

    def test_catalog_rows_and_bold_spans_merge_in_display_order(
        self, cart_agent: CartAgent
    ) -> None:
        context = (
            "Here are some options:\n"
            "**Ginger Lace Trim Gown** -- elegant.\n"
            "Ginger Lace Trim Gown | a gown | apparel\n"
            "1. Luminous Satin Dress | shimmering | apparel\n"
            "2. Velvet Evening Gown | plush | apparel\n"
        )
        assert CartAgent._extract_display_order(context) == [
            "Ginger Lace Trim Gown",
            "Luminous Satin Dress",
            "Velvet Evening Gown",
        ]
    ...
    def test_dedupes_case_insensitively_keeping_first_casing(
        ...
        context = (
            "**Silk Dress** mentioned early\n"
            "silk dress | smooth | apparel\n"
            "**Silk Dress** again\n"
        )
        assert CartAgent._extract_display_order(context) == ["Silk Dress"]

    def test_non_product_spans_excluded(self, cart_agent: CartAgent) -> None:
        context = (
            "**Price: $69.99** is steep.\n"
            "**Summary**\n"
            "Silk Dress | lovely | apparel\n"
        )
        assert CartAgent._extract_display_order(context) == ["Silk Dress"]
...
class TestResolveTargetItemOrdinals:
    """Ordinal references must resolve by display order, not rightmost hit."""

    CONTEXT = (
        "Here are some red dresses:\n"
        "1. **Ginger Lace Trim Gown** -- $179.99\n"
        "2. **Luminous Satin Dress** -- $149.99\n"
        "3. **Velvet Evening Gown** -- $209.99\n"
    )

    @pytest.mark.parametrize(
        "query,expected",
        [
            ("add the first one to my cart", "Ginger Lace Trim Gown"),
            ("add the second one to my cart", "Luminous Satin Dress"),
            ("I'll take the third one", "Velvet Evening Gown"),
            ("the last one is nice, add it", "Velvet Evening Gown"),
            ("grab the 2nd dress", "Luminous Satin Dress"),
            ("take the 1st one", "Ginger Lace Trim Gown"),
        ],
    )
    def test_ordinal_positions_resolved_in_display_order(
        self, cart_agent: CartAgent, query: str, expected: str
    ) -> None:
        state = State(user_id=1, query=query, cart=Cart(contents=[]), context=self.CONTEXT)
        assert cart_agent._resolve_target_item(state) == expected

    def test_ordinal_takes_precedence_over_pronoun_fallback(
        self, cart_agent: CartAgent
    ) -> None:
        # Regression: "the first one" used to fall into the pronoun branch
        # (its bare "one" token) and resolve to the *last-mentioned*
        # product instead of the first-displayed one.
        state = State(
            user_id=1,
        ...
        resolved, kind = cart_agent._resolve_target_item_with_kind(state)
        assert (resolved, kind) == ("Ginger Lace Trim Gown", "ordinal")
    ...
    def test_ordinal_out_of_range_returns_none(
        ...
        state = State(
            user_id=1,
            query="add the fifth one to my cart",
            cart=Cart(contents=[]),
            context=self.CONTEXT,
        )
        assert cart_agent._resolve_target_item(state) is None

    def test_ordinal_with_empty_context_returns_none(
        self, cart_agent: CartAgent
    ) -> None:
        state = State(user_id=1, query="add the first one to my cart")
        assert cart_agent._resolve_target_item(state) is None

    def test_named_product_still_wins_over_ordinal(
        self, cart_agent: CartAgent
    ) -> None:
        state = State(
            user_id=1,
            query="add the first Velvet Evening Gown to my cart",
            cart=Cart(contents=[]),
            context=self.CONTEXT,
        )
        resolved, kind = cart_agent._resolve_target_item_with_kind(state)
        assert (resolved, kind) == ("Velvet Evening Gown", "named")

    def test_bare_pronoun_still_uses_last_mentioned(
        self, cart_agent: CartAgent
    ) -> None:
        state = State(
            user_id=1,
        ...
        resolved, kind = cart_agent._resolve_target_item_with_kind(state)
        assert (resolved, kind) == ("Velvet Evening Gown", "pronoun")


class TestCartAgentInvokeOverrideContract:
    """The deterministic resolver must respect anchor strength vs the LLM."""
    ...
    def _state(self, query: str) -> State:
        return State(user_id=7, query=query, cart=Cart(contents=[]), context=self.CONTEXT)

    def _catalog_lookup_names(self, recorder: _Recorder) -> List[List[str]]:
        """Names sent to the search service (post-resolution, pre-match)."""
        return [
            payload["text"]
            for _, url, payload in recorder.calls
            if url.endswith("/query/text") and "text" in payload
        ]

    def test_llm_product_field_kept_for_pronoun_query(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # Regression for the Round-4 root cause: the LLM correctly picked
        # Ginger Lace Trim Gown; the bare-pronoun fallback must NOT
        # override an explicit LLM choice.
        recorder = _install_http_stubs(
            monkeypatch,
            cart_before=[],
            add_message="added 1 Ginger Lace Trim Gown",
        )
        _install_llm_tool_response(
            cart_agent,
            "add_to_cart",
            {"product": "Ginger Lace Trim Gown", "quantity": 1},
        )

        cart_agent.invoke(self._state("add it to my cart"), verbose=False)

        # The resolver kept the LLM pick: the catalog was queried with it.
        assert self._catalog_lookup_names(recorder) == [["Ginger Lace Trim Gown"]]

    def test_ordinal_overrides_wrong_llm_pick(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # The user said "the first one"; even if the LLM names a different
        # product from context, the explicit ordinal instruction wins.
        recorder = _install_http_stubs(
            monkeypatch,
            cart_before=[],
            add_message="added 1 Ginger Lace Trim Gown",
        ...
        _install_llm_tool_response(
            cart_agent,
            "add_to_cart",
            {"item_name": "Luminous Satin Dress", "quantity": 1},
        )

        cart_agent.invoke(self._state("add the first one to my cart"), verbose=False)

        # Ordinal resolution overrode the wrong LLM pick before lookup,
        # and the add still went through to the memory service.
        assert self._catalog_lookup_names(recorder) == [["Ginger Lace Trim Gown"]]
        assert any(url.endswith("/cart/add") for _, url, _p in recorder.calls)

    def test_missing_llm_field_filled_from_last_mentioned(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        recorder = _install_http_stubs(
            monkeypatch,
            cart_before=[],
            add_message="added 1 Velvet Evening Gown",
        )
        _install_llm_tool_response(cart_agent, "add_to_cart", {"quantity": 1})

        cart_agent.invoke(self._state("add it to my cart"), verbose=False)

        # Missing item_name is filled from the last-mentioned product.
        assert self._catalog_lookup_names(recorder) == [["Velvet Evening Gown"]]

    def test_chat_heading_never_overrides_llm_pick(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # PM review 2026-08-24, item 13 regression (exact field scenario):
        # the user asks to "add a silk scarf to my cart"; the LLM correctly
        # parses the target, but the running context's most recent line is
        # a chatter heading "**Manage your cart**". The deterministic
        # resolver must NOT turn that heading into a "named" anchor that
        # replaces the LLM's pick with prose.
        recorder = _install_http_stubs(
            monkeypatch,
            cart_before=[],
            add_message="added 1 silk scarf",
        )
        _install_llm_tool_response(
            cart_agent,
            "add_to_cart",
            {"item_name": "silk scarf", "quantity": 1},
        )
        state = State(
            user_id=7,
            query="add a silk scarf to my cart",
            cart=Cart(contents=[]),
            context=(
                "Here are some options:\n"
                "1. **Ginger Lace Trim Gown** -- $179.99\n"
                "2. **Luminous Satin Dress** -- $149.99\n"
                "Agent Response: **Manage your cart**\n"
                "I can help you manage your cart. What would you like to do?"
            ),
        )

        cart_agent.invoke(state, verbose=False)

        # The LLM's pick survives; the catalog was queried with it, not
        # with the harvested heading text.
        assert self._catalog_lookup_names(recorder) == [["silk scarf"]]
        assert any(url.endswith("/cart/add") for _, url, _p in recorder.calls)
...
def _install_http_stubs(
    ...
    * ``requests.Session().post`` to the search service for name lookups.
    * ``requests.get`` / ``requests.post`` to the memory service for cart CRUD.
    ...
    catalog_response = catalog_response or {
        ...
        "urls": ["https://example.com/silk-dress"],
    }
    ...
    monkeypatch.setattr(catalog_match.requests, "Session", _fake_session_ctor)
    monkeypatch.setattr(_FakeSession, "post", _fake_session_post)

    monkeypatch.setattr(memory_client.requests, "get", _fake_requests_get)
    monkeypatch.setattr(memory_client.requests, "post", _fake_requests_post)
    ...
    if cart_after is not None:
        ...
        monkeypatch.setattr(memory_client.requests, "get", _fake_requests_get_after)
...
class TestLookupInCatalog:
    def test_returns_match_dict_on_success(
        ...
        assert match["url"] == "https://example.com/silk-dress"
...
class TestAddRemoveCart:
    def test_add_calls_memory_with_price_and_url_when_available(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        recorder = _install_http_stubs(
            monkeypatch,
        ...
        assert payload == {
            ...
            "amount": 2,
            "price": 49.99,
            "url": "https://example.com/silk-dress",
        }

    def test_add_sends_empty_url_when_catalog_url_is_missing(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        recorder = _install_http_stubs(
            monkeypatch,
            catalog_response={
                "names": ["Silk Dress"],
                "similarities": [0.95],
                "texts": ["Silk Dress | dress\nPRICE: 49.99"],
            },
        )

        cart_agent._add_to_cart(user_id=42, item_name="Silk Dress", quantity=1)

        add_payload = next(
            payload for kind, endpoint, payload in recorder.calls
            if kind == "POST" and endpoint.endswith("/cart/add")
        )
        assert add_payload["url"] == ""
    ...
    def test_add_rejects_non_https_catalog_url(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch, url: str
    ) -> None:
        recorder = _install_http_stubs(
            monkeypatch,
            catalog_response={
...
class TestCartAgentInvoke:
    ...
    def test_pronoun_query_keeps_llm_pick_over_last_mentioned(
        self, cart_agent: CartAgent, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        # Pronoun references are a weak signal: when the LLM made an
        # explicit choice from the same context, it is kept even if it
        # differs from the last-mentioned product (the historical bug
        # where the rightmost mention silently replaced a correct LLM
        # pick must not come back).
        ...
        # The LLM pick survives; no silent rewrite to last-mentioned.
        catalog_lookups = [
            payload["text"]
            for _, url, payload in recorder.calls
            if url.endswith("/query/text") and "text" in payload
        ]
        assert catalog_lookups == [["Hiking Boot"]]
...
def _install_bulk_http_stubs(
    ...
    *,
    ...
    lookup. For bulk tests we need the search service mock to return a
...
class TestBulkCartDispatch:
    ...
    def test_bulk_add_accepts_stringified_items_from_xml_fallback(
        ...
        # The XML tool-call path delivers ``items`` as a string repr.
