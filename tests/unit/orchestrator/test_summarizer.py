...
"""Unit tests for ``orchestrator.app.summarizer``.
...
from orchestrator.app.agents.summarizer import SummaryAgent
...
def summary_agent(base_config, monkeypatch: pytest.MonkeyPatch) -> SummaryAgent:
    ...
    tiny_config = SimpleNamespace(
        ...
        memory_length=50,
        memory_base_url=base_config.memory_base_url,
    )
...
class TestSummaryAgentInvoke:
    ...
    def test_memory_base_url_url_is_constructed_per_user(
        self, summary_agent: SummaryAgent, post_recorder: _PostRecorder
    ) -> None:
        ...
        call = post_recorder.calls[0]
        assert call.url.startswith(summary_agent.memory_base_url)
