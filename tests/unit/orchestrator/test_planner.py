...
"""Unit tests for ``orchestrator.app.planner``.
...
from orchestrator.app.agents.planner import PlannerAgent
...
class TestInvoke:
    @pytest.mark.parametrize(
        ...
        [
            ...
            "How much did I spend on orders this month?",
            "这个月花了多少",
            "帮我看看本月花费",
        ],
    )
    def test_spending_recall_queries_bypass_the_llm(
        self, planner_agent: PlannerAgent, query: str
    ) -> None:
        def _explode(**_: Any) -> Any:
            raise AssertionError("Spending-recall queries must route deterministically")
    ...
    @pytest.mark.parametrize(
        "query",
        [
            "show me dresses to spend under $100",
            "I want to spend less than $100",
            "spend under $100 on a dress",
        ],
    )
    def test_budget_discovery_queries_use_llm_routing(
        self, planner_agent: PlannerAgent, query: str
    ) -> None:
        _stub_llm(planner_agent, "retriever")
        state = State(user_id=1, query=query)
    ...
    def test_verbose_cart_justification_is_salvaged_as_cart(
        self, planner_agent: PlannerAgent
    ) -> None:
        verbose = (
            "the user wants to add the first one to their cart. this is a cart "
            "operation, so route to cart_node."
        )
        _stub_llm(planner_agent, verbose)
        state = State(user_id=1, query="Add the first one to my cart")

        out = planner_agent.invoke(state, verbose=False)

        assert out.next_agent == "cart"

    def test_verbose_discovery_justification_is_salvaged_as_retriever(
        self, planner_agent: PlannerAgent
    ) -> None:
        verbose = (
            "the user wants to buy a red dress and see options. this is "
            "product discovery, so it falls under search."
        )
        _stub_llm(planner_agent, verbose)
        state = State(user_id=1, query="Show me some red dresses")
