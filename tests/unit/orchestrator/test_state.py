...
"""Unit tests for ``orchestrator.app.agenttypes``.

These tests pin the data contract the LangGraph pipeline relies on:
``Cart`` helpers, ``State`` default + helper behaviour, and ``SafetyResult`` timing
...
from orchestrator.app.agents.state import Cart, SafetyResult, State
...
class TestSafetyResult:
    def test_safety_result_defaults_to_safe_with_empty_timings(self) -> None:
        safety_result = SafetyResult()

        assert safety_result.is_safe is True
        assert safety_result.safety_timings == {}
        assert safety_result.get_total_safety_time() == 0.0

    def test_add_timing_aggregates_per_check(self) -> None:
        safety_result = SafetyResult(is_safe=False)

        safety_result.add_timing("input", 0.02)
        safety_result.add_timing("output", 0.03)

        assert safety_result.is_safe is False
        assert safety_result.get_total_safety_time() == pytest.approx(0.05)

    def test_total_safety_time_handles_overwrite_of_same_key(self) -> None:
        safety_result = SafetyResult()

        safety_result.add_timing("input", 0.05)
        safety_result.add_timing("input", 0.02)

        assert safety_result.safety_timings == {"input": 0.02}
        assert safety_result.get_total_safety_time() == pytest.approx(0.02)
