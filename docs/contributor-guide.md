...
1. **文件保持小而专一**：业务文件 ≤200 行，util 30-80 行；超了按职责拆分
2. **模块依赖无环**：跨服务只走 HTTP，不互相 import
3. **测试跟随**：拆/改模块时同步更新 `tests/unit/<服务>/` 下对应测试
4. **i18n**：web 用户可见文案进 `src/i18n/{en,zh}.json`，两语言同步
5. **无版权头**：源文件不放 SPDX/版权声明头（保持现状）
